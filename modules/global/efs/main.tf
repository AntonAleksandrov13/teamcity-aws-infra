...
resource "aws_efs_file_system" "k8s_efs" {
  tags                            = { "Name" = "${var.cluster_name}-k8s-efs" }
  encrypted                       = true
  kms_key_id                      = aws_kms_key.k8s_efs.0.arn
  throughput_mode                 = var.throughput_mode
  provisioned_throughput_in_mibps = var.provisioned_throughput_in_mibps
}

resource "aws_efs_mount_target" "k8s_efs" {
  file_system_id = aws_efs_file_system.k8s_efs.0.id

  subnet_id       = var.eks_worker_subnets
  security_groups = [aws_security_group.k8s_efs.0.id]
}
...
resource "aws_security_group_rule" "ingress_efs_k8s_workers" {
  security_group_id        = aws_security_group.k8s_efs.0.id
  type                     = "ingress"
  from_port                = 2049
  to_port                  = 2049
  protocol                 = "tcp"
